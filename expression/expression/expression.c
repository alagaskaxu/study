...
int main()
{
	...
	//算术转换
	//如果某个操作符的各个操作符属于不同的类型,那么除非其中一个操作数的类型转换为另一个操作数的类型,否则操作无法进行.
	//下面的体系称为寻常算术转换
	/*
	* long double
	* double
	...
	* int
	*/
	//操作数的类型在上面的列表中排名较低,那么首先转换为另外一个操作数的类型后执行运算(向上转换,类型小的转换为大的)
	a = 4;
	float f = 4.5f;
	f = a + f;	//a 为int精度比float小(在列表最下,向上转换),a的int类型转换为float类型进行运算
	printf("f = %f", f);	//4.000000+4.5000000 = 8.500000

	//操作符的属性
	/*
	* 1.操作符的优先级(决定计算顺序) : ()优先级最高
	* 2.操作符的结合性 : 优先级不起作用,则由结合性决定顺序
	* 3.是否控制求值顺序 :只有几个操作符能控制求值顺序:  && || 条件操作符 ? : (三目操作符)  逗号表达式 ,
	*/
